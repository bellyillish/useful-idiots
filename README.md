...
Overhauls the companion system for Anomaly and G.A.M.M.A. Does not (and never will) overwrite core files to ensure it stays as compatible as possible with other mods. Features include:

- [Over 50 commands](#commands) to direct and interact with your companions
- A [Customizable UI](#new-companion-ui) to replace the companion wheel
- Extensive [keyboard support](#extensive-keyboard-support) for every command (powered by MCM)
- AI [improvements and bug fixes](#improvements-and-fixes) for companions and NPCs
<!-- -->
...
- **NPC Stops Dropping Weapons and Looting Dead Bodies** interferes with looting and gathering. Instead, an MCM option is included that does the exact same thing. By default it is enabled if running G.A.M.M.A. and disabled otherwise. The same *possibly* goes for any mod that attempts to alter NPC looting and gathering behavior.

- **Companion Anti AWOL for Anomaly:** doesn't conflict but can get in the way. You'll have plenty of control over your idiots, and this mod can leave them vulnerable by making them de-aggro in front of enemies at inopportune times. Ultimately it's your call but keep it in mind.

- **He Is With Me:** also doesn't conflict but is redundant. A replacement is included with [some modifications](#he-is-with-me) to the original mod's behavior.

- **Settings -> Gameplay -> General:** If you have "Only Companions Can Loot and Gather Items" enabled in mod settings, I suggest turning "Corpse Loot Distance" down to 0-2 meters. It tends to confuse people especially at first. Otherwise the default is fine in Anomaly but a bit high in G.A.M.M.A. (I'd change it to something like 5-6 meters).

## Instructions
- Download the ZIP file here or from [GitHub Releases](https://github.com/bellyillish/useful-idiots/releases).
- Install it with [Mod Organizer 2](https://anomalymodding.blogspot.com/2021/04/Mod-Organizer-2-setup-and-Amomaly-modding-guide.html). It's included with G.A.M.M.A. but I couldn't imagine modding Anomaly without it. Priority is not important for Anomaly. It should be fine anywhere below the mods included with G.A.M.M.A.
- Go to **Mod Configuration Menu -> Useful Idiots** to customize and configure to your liking.
<!-- -->
...
The new UI replaces the base game's companion wheel and is opened via the same keybind. Features of the new UI include:
...
When pointing at an idiot, the UI opens to that idiot's tab. When pointing at nobody, the UI opens to the "All" tab which lets you issue global comands to all idiots.
<!-- -->

<!-- -->
## Extensive Keyboard Support

Every available command can be assigned a keybind with ALT/CTRL/SHIFT modifiers. Separate keybinds can also be assigned to cycle through groups of related commands. There are also a [subset of commands](#keyboard-only-commands) that are *only* available though keybinds.
<!-- -->
