...
-- ZONES --
  ...
  function COMBAT.getTargetZone(self)
    ...
    if IsStalker(be) and be:character_rank() > npc:character_rank() * 2
      then targetZone = targetZone + 1
    elseif state.recovering
      then targetZone = targetZone + 1
    end
...
-- CALLBACKS --
  function COMBAT.combatHitCallback(self, obj, amount, direction, who, bone)
    ...
    if not (obj:id() == npc:id() and who and who:id() == enemy.id) then
      return
    end
  ...
  function COMBAT.combatHearCallback(self, obj, whoid, type, position, power)
    ...
    if not (obj:id() == npc:id() and enemy.id == whoid) then
      return
    end
